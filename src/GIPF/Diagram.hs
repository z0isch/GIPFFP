...
gridLabels' = bottom ++ top
  where
    ...
    top = fixSpacing (-0.5) $
            zip (scanl' getNeighbor (-4,-1) (concatMap (replicate 4) [UpRightD,DownRightD])) (zipWith (\a b -> a : show b) ['a'..] ([5..9] ++[8,7,6,5]))
...
testGame :: IO ()
testGame = do
  mMoves <- parseFromFile gipfGameParser "testGame2.gipf"
  case mMoves of
    ...
    Just moves ->   let flatMoves :: [(Player,PieceMove)]
                        ...
                        game =  scanl' (\(g,_) (pl,m) -> (join $ makeMove pl <$> g <*> pure m, Just m)) (pure emptyGrid,Nothing) flatMoves
                        gameDiagrams = zip [1..] $ map (first (fmap gridDiagram)) game
...
pieceMoveDia :: Maybe PieceMove -> QDiagram Rasterific V2 Double Any
pieceMoveDia Nothing = text ""
pieceMoveDia (Just m) = text note # fontSizeL 0.3
  where note = foldAcc $ pieceMoveToNotation m

foldAcc :: AccValidation [String] String -> String
